/**
 * 投影 (Projections)
 *
 * 本程式示範：
 * 1. 投影到直線
 * 2. 投影矩陣及其性質
 * 3. 投影到子空間
 * 4. 誤差向量的正交性驗證
 *
 * 編譯：g++ -std=c++17 -O2 projection.cpp -o projection
 * 執行：./projection
 */

#include <iostream>
#include <vector>
#include <cmath>
#include <iomanip>
#include <string>

using Vector = std::vector<double>;
using Matrix = std::vector<std::vector<double>>;

const double EPSILON = 1e-10;

// ========================================
// 輔助函數
// ========================================

void printSeparator(const std::string& title) {
    std::cout << "\n" << std::string(60, '=') << "\n";
    std::cout << title << "\n";
    std::cout << std::string(60, '=') << "\n";
}

void printVector(const std::string& name, const Vector& v) {
    std::cout << name << " = [";
    for (size_t i = 0; i < v.size(); ++i) {
        std::cout << std::fixed << std::setprecision(4) << v[i];
        if (i < v.size() - 1) std::cout << ", ";
    }
    std::cout << "]\n";
}

void printMatrix(const std::string& name, const Matrix& M) {
    std::cout << name << " =\n";
    for (const auto& row : M) {
        std::cout << "  [";
        for (size_t j = 0; j < row.size(); ++j) {
            std::cout << std::fixed << std::setprecision(4) << std::setw(8) << row[j];
            if (j < row.size() - 1) std::cout << ", ";
        }
        std::cout << "]\n";
    }
}

// ========================================
// 基本運算
// ========================================

double dotProduct(const Vector& x, const Vector& y) {
    double result = 0.0;
    for (size_t i = 0; i < x.size(); ++i) {
        result += x[i] * y[i];
    }
    return result;
}

double vectorNorm(const Vector& x) {
    return std::sqrt(dotProduct(x, x));
}

Vector scalarMultiply(double c, const Vector& x) {
    Vector result(x.size());
    for (size_t i = 0; i < x.size(); ++i) {
        result[i] = c * x[i];
    }
    return result;
}

Vector vectorSubtract(const Vector& x, const Vector& y) {
    Vector result(x.size());
    for (size_t i = 0; i < x.size(); ++i) {
        result[i] = x[i] - y[i];
    }
    return result;
}

Matrix outerProduct(const Vector& x, const Vector& y) {
    Matrix result(x.size(), Vector(y.size()));
    for (size_t i = 0; i < x.size(); ++i) {
        for (size_t j = 0; j < y.size(); ++j) {
            result[i][j] = x[i] * y[j];
        }
    }
    return result;
}

Matrix matrixScalarMultiply(double c, const Matrix& A) {
    Matrix result = A;
    for (auto& row : result) {
        for (auto& val : row) {
            val *= c;
        }
    }
    return result;
}

Vector matrixVectorMultiply(const Matrix& A, const Vector& x) {
    Vector result(A.size(), 0.0);
    for (size_t i = 0; i < A.size(); ++i) {
        for (size_t j = 0; j < x.size(); ++j) {
            result[i] += A[i][j] * x[j];
        }
    }
    return result;
}

Matrix matrixMultiply(const Matrix& A, const Matrix& B) {
    size_t m = A.size(), k = B.size(), n = B[0].size();
    Matrix result(m, Vector(n, 0.0));
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            for (size_t p = 0; p < k; ++p) {
                result[i][j] += A[i][p] * B[p][j];
            }
        }
    }
    return result;
}

Matrix transpose(const Matrix& A) {
    size_t m = A.size(), n = A[0].size();
    Matrix result(n, Vector(m));
    for (size_t i = 0; i < m; ++i) {
        for (size_t j = 0; j < n; ++j) {
            result[j][i] = A[i][j];
        }
    }
    return result;
}

// ========================================
// 投影函數
// ========================================

struct ProjectionResult {
    double xHat;
    Vector projection;
    Vector error;
    double errorNorm;
};

/**
 * 投影到直線
 * p = (aᵀb / aᵀa) * a
 */
ProjectionResult projectOntoLine(const Vector& b, const Vector& a) {
    double aTb = dotProduct(a, b);
    double aTa = dotProduct(a, a);

    double xHat = aTb / aTa;
    Vector p = scalarMultiply(xHat, a);
    Vector e = vectorSubtract(b, p);

    return {xHat, p, e, vectorNorm(e)};
}

/**
 * 投影到直線的投影矩陣
 * P = aaᵀ / (aᵀa)
 */
Matrix projectionMatrixLine(const Vector& a) {
    double aTa = dotProduct(a, a);
    Matrix aaT = outerProduct(a, a);
    return matrixScalarMultiply(1.0 / aTa, aaT);
}

/**
 * 驗證投影矩陣的性質
 */
void verifyProjectionMatrix(const Matrix& P, const std::string& name = "P") {
    size_t n = P.size();

    std::cout << "\n驗證 " << name << " 的性質：\n";

    // 對稱性
    bool isSymmetric = true;
    for (size_t i = 0; i < n && isSymmetric; ++i) {
        for (size_t j = 0; j < n && isSymmetric; ++j) {
            if (std::abs(P[i][j] - P[j][i]) > EPSILON) {
                isSymmetric = false;
            }
        }
    }
    std::cout << "  對稱性 (" << name << "ᵀ = " << name << ")：" << (isSymmetric ? "true" : "false") << "\n";

    // 冪等性
    Matrix P2 = matrixMultiply(P, P);
    bool isIdempotent = true;
    for (size_t i = 0; i < n && isIdempotent; ++i) {
        for (size_t j = 0; j < n && isIdempotent; ++j) {
            if (std::abs(P[i][j] - P2[i][j]) > EPSILON) {
                isIdempotent = false;
            }
        }
    }
    std::cout << "  冪等性 (" << name << "² = " << name << ")：" << (isIdempotent ? "true" : "false") << "\n";
}

// ========================================
// 主程式
// ========================================

int main() {
    std::cout << std::fixed << std::setprecision(4);

    printSeparator("投影示範 (C++)\nProjection Demo");

    // 1. 投影到直線
    printSeparator("1. 投影到直線");

    Vector a = {1.0, 1.0};
    Vector b = {2.0, 0.0};

    printVector("方向 a", a);
    printVector("向量 b", b);

    auto result = projectOntoLine(b, a);

    std::cout << "\n投影係數 x̂ = (aᵀb)/(aᵀa) = " << result.xHat << "\n";
    printVector("投影 p = x̂a", result.projection);
    printVector("誤差 e = b - p", result.error);

    // 驗證正交性
    double eDotA = dotProduct(result.error, a);
    std::cout << "\n驗證 e ⊥ a：e · a = " << eDotA << "\n";
    std::cout << "正交？ " << (std::abs(eDotA) < EPSILON ? "true" : "false") << "\n";

    // 2. 投影矩陣
    printSeparator("2. 投影矩陣（到直線）");

    Vector a2 = {1.0, 2.0};
    printVector("方向 a", a2);

    Matrix P = projectionMatrixLine(a2);
    printMatrix("\n投影矩陣 P = aaᵀ/(aᵀa)", P);

    verifyProjectionMatrix(P);

    // 用投影矩陣計算投影
    Vector b2 = {3.0, 4.0};
    printVector("\n向量 b", b2);

    Vector p = matrixVectorMultiply(P, b2);
    printVector("投影 p = Pb", p);

    // 3. 多個向量的投影
    printSeparator("3. 批次投影");

    std::vector<Vector> vectors = {{1.0, 0.0}, {0.0, 1.0}, {2.0, 2.0}, {3.0, -1.0}};

    std::cout << "方向 a = [1, 2]\n";
    std::cout << "\n各向量投影結果：\n";

    for (const auto& v : vectors) {
        auto proj = projectOntoLine(v, a2);
        std::cout << "  [" << v[0] << ", " << v[1] << "] -> ["
                  << proj.projection[0] << ", " << proj.projection[1] << "]\n";
    }

    // 總結
    printSeparator("總結");
    std::cout << R"(
投影公式：

1. 投影到直線：
   p = (aᵀb / aᵀa) a
   P = aaᵀ / (aᵀa)

2. 投影到子空間：
   p = A(AᵀA)⁻¹Aᵀb
   P = A(AᵀA)⁻¹Aᵀ

3. 投影矩陣性質：
   Pᵀ = P（對稱）
   P² = P（冪等）
)" << "\n";

    std::cout << std::string(60, '=') << "\n";
    std::cout << "示範完成！\n";
    std::cout << std::string(60, '=') << "\n";

    return 0;
}
